...
def parse_process_info(file_content):
    process_info_pattern = re.compile(r'----- pid (?P<pid>\d+) at (?P<timestamp>[\d\-:\.\+ ]+)\s+-----\nCmd line: (?P<cmd_line>.*)\nBuild fingerprint: (?P<build_fingerprint>.*)\nABI: (?P<abi>.*)\nBuild type: (?P<build_type>.*)\nsuspend all histogram:.*', re.DOTALL)
    match = process_info_pattern.search(file_content)
...
def parse_thread_stack(thread_content):
    # Extract thread name
    ...
    thread_name = thread_name_match.group('thread_name') if thread_name_match else "Unknown"

    # Extract thread metadata (| lines)
    metadata = {}
    metadata_pattern = re.compile(r'^\s*\|\s*(?P<key>[\w\s]+)=\s*(?P<value>.*?)$', re.MULTILINE)
    ...
    lock_info = []
    lock_pattern = re.compile(r'^\s*- (waiting on|waiting to lock|locked)\s+<(?P<lock_address>[0-9a-f]+)>', re.MULTILINE)
...
def parse_threads(file_content):
    ...
    for thread_content in thread_split_pattern.finditer(file_content):
        ...
        thread_name, metadata, frames, lock_info = parse_thread_stack(content)
...
def display_thread_and_lock_info(process_info, threads, waiting_threads, holding_threads):
    ...
    for lock_address in set(waiting_threads.keys()).union(set(holding_threads.keys())):
        ...
        if waiting:
            print(f"Lock 0x{lock_address} is being waited on by: {', '.join(waiting)}")
        print("-" * 40)
...
"""
    ...
    display_thread_and_lock_info(process_info, threads, waiting_threads, holding_threads)
